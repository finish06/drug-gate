...
- Production deploy automation for K3s: `deploy-prod` workflow (tag `v*` → `production` Environment approval → `kubectl set image` → rollout health gate → k6 smoke → auto `rollout undo` on failure) and `rollback-prod` workflow (`workflow_dispatch`, optional `to_revision`). Runbook at `ops/production-deploy.md` (deploy, rollback, Redis recovery, breaker reset). Runs on a self-hosted in-cluster runner; namespace `drugs`. (deploy-automation AC-001…AC-016)
