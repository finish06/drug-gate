...
### Added
- `GET /livez` — dependency-free liveness probe (always 200 if the process is up). Unlike `/health` (which returns 503 when Redis is down), `/livez` never checks dependencies, so it can serve as the Kubernetes liveness probe without crash-looping pods during a Redis outage. Readiness stays on `/health`. (deploy-automation AC-006)
- Production deploy notification: a `notify-prod` job in `ci.yml` runs after `publish` on `v*` tags and POSTs a NATS event (subject `joe.deploy.drug-gate.tag.<tag>`) carrying the exact image digest, so the homelab agent can prompt a human in Discord to promote. The agent owns the cluster deploy/health-gate/rollback; this repo only announces. Sequenced publish→notify (no registry-poll race); fails CI on a non-202 bridge response. Runbook at `ops/production-deploy.md`. (deploy-automation AC-001…AC-011)
