...
- Production deploy notification: `notify-prod-promote` workflow publishes a NATS event on a `v*.*.*` tag (subject `joe.deploy.drug-gate.tag.<version>`) so the homelab agent can prompt a human in Discord to promote. The agent owns the cluster deploy/health-gate/rollback; this repo only announces. Runbook at `ops/production-deploy.md`. (deploy-automation AC-001…AC-011)
